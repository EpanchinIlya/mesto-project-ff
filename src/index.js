...
import { createCard, deleteCard,toggleLikeCallBackFunction} from "./components/card.js";
import { openModal,closeModal, addpopupCloseToEscEventListeners } from "./components/modal.js";
...
const popup_type_image__caption = popup_type_image.querySelector(".popup__caption")
...
function addAllCards(initialCards) {
  initialCards.forEach((item) =>
    placesList.append(
      createCard(item, deleteCard, toggleLikeCallBackFunction, createPopupCallBackFunction))
  );
}
...
function addOneCard(card){
  placesList.prepend( createCard(card, deleteCard, toggleLikeCallBackFunction, createPopupCallBackFunction)
  );}
...
function createPopupCallBackFunction(evt) {
  ...
  popup_type_image.querySelector(".popup__close").addEventListener("click", closeModal);
}
