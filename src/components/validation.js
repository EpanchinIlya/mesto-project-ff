...
//       }
      ...
      //  debugger
        ...
        formList.forEach((formElement) => {
            ...
            formElement.addEventListener('submit', function (evt) { evt.preventDefault(); });
             ...
             const inputList = Array.from(formElement.querySelectorAll(objVal.inputSelector));

              inputList.forEach((inputElement) => {
               inputElement.addEventListener('input', function () {

                checkInputValidity(formElement, inputElement, objVal.inputErrorClass, objVal.errorClass);
               // checkInputValidity(formElement, inputElement, objVal.inputErrorClass, objVal.errorClass);
   ...
   const clearValidation = (profileForm, objVal) =>{
      ...
      const inputList = profileForm.querySelectorAll(objVal.inputSelector);
            inputList.forEach((item) => { item.classList.remove(objVal.inputErrorClass)});
           
      const submitButton = profileForm.querySelector(objVal.submitButtonSelector);


           if(Array.from(inputList).every(item=>{ return(item.value.length>0)})){
